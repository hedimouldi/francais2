...
const Museum = {
    ...
    /**
     * Create floor material (high standing marble)
     */
    _createFloorMaterial(type, width, depth) {
        const loader = new THREE.TextureLoader();
        const texture = loader.load('assets/textures/floor_marble.png');
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(Math.max(1, width / 8), Math.max(1, depth / 8));
        texture.colorSpace = THREE.SRGBColorSpace;

        return new THREE.MeshStandardMaterial({
            map: texture,
            roughness: 0.1, // Shiny luxury marble
            metalness: 0.1,
            // Hall (was carpet) uses pure white marble, rooms (were wood) use slightly warm marble
            color: type === 'carpet' ? 0xffffff : 0xfffcf0,
        });
    },
